...
def zone_populations():
    sheet = pd.read_excel(os.getcwd() + "/dbs/saopaulo/population.xlsx", skiprows=6, index_col=0, skipfooter=17, sheet_name=0)
...
def zones():
    _zones = gpd.read_file(os.getcwd() + '/dbs/saopaulo/zones/zones.shp')
...
def odm():
    odms = pd.read_excel(os.getcwd() + "/dbs/saopaulo/odm/odm.xlsx", skiprows=7, index_col=0, skipfooter=3).drop(columns="Total")

...
# Objective : Take the grid search record and visualise it - Sao Paulo
...
sc <- scale_colour_gradientn(colours = myPalette(100), limits=c(0, 0.02), name = 'KL divergence')
...
g1 <- ggplot(df, aes(x=beta, y=gamma, color=national)) +
  ...
  sc +
  labs(title = 'São Paulo', subtitle = TeX("$\\rho$"),
